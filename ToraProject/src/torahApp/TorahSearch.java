...
public class TorahSearch {
	private static TorahSearch instance;

	public static TorahSearch getInstance() {
		if (instance == null) {
			instance = new TorahSearch();
		}
		return instance;
	}
...
//				outputStream.getBuffer().setLength(0);
			...
			if (count > 0) {
				ExcelFunctions.writeXLS(fileName, sheet, (bool_sofiot) ? 0 : 1, Title, results, Title2, Title3,
						searchSTR, searchSTR2, ((TorahApp.isGui()) ? Frame.get_searchRangeText() : ""));
			}
		} catch (
