...
public class SwingActivity extends SwingWorker<Void, Integer> {
	...
	protected Void doInBackground() {
		...
		if (mainComboString != Frame.combo_strTorahPrint) {
			...
			if ((!differentSearch) && (mainComboString.equals(Frame.combo_strDilugim)) && (searchSTR1.length() == 1)) {
		...
		case Frame.combo_strLetterSearch:
		case Frame.combo_strSearch:
			Frame.setBool_CanStore(true);
			break;
		default:
			Frame.setBool_CanStore(false);
		}
		...
		try {
			...
			case Frame.combo_strSearch:
				...
				if ((Frame.getCheckbox_searchMultiple()) && (searchSTR2.length() > 0)) {
					args[5] = searchSTR2;
			...
			case Frame.combo_strDilugim:
				...
				if (!StringUtils.isNumeric(searchSTR2.trim())) {
